...
final class MorphKeyRegistry
{
     ...
     *
     * Like map(), but also enables strict enforcement. Any model used
     ...
     * $registry->getKey($user);
     * $registry->getKey($seller);
     *
     * // This will throw MorphKeyViolationException
     * $registry->getKey($post); // Post not in mapping
     * ```
     *
     * @param array<class-string, string> $map Model class => column name mappings
     */
     ...
     * Typically used via enforce() rather than directly.
     */
    ...
    /**
     * Check if a model class has a registered key mapping.
     *
     * @param class-string|Model $model Model instance or class name
     */
     ...
     *
     * Similar to getKey() but accepts a class string instead of an instance.
    ...
    /**
     * Get all registered key mappings.
     *
